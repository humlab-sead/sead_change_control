...
begin
    v_submission_id := (select submission_id from clearing_house.tbl_clearinghouse_submissions where submission_name = '20250108_DML_SUBMISSION_ADNA_PILOT_COMMIT' limit 1);
...
create temp table staging_tbl_clearinghouse_submissions (like clearing_house.tbl_clearinghouse_submissions including defaults);

\copy staging_tbl_clearinghouse_submissions (submission_id, submission_name, source_name, submission_state_id, data_types, upload_user_id, upload_date, status_text, claim_user_id, claim_date_time, submission_uuid) from program 'zcat -qac tbl_clearinghouse_submissions.sql.gz' with (format text, delimiter E'\t', encoding 'utf-8');

insert into clearing_house.tbl_clearinghouse_submissions (submission_id, submission_name, source_name, submission_state_id, data_types, upload_user_id, upload_date, status_text, claim_user_id, claim_date_time, submission_uuid)
    select :'submission_id', submission_name, source_name, submission_state_id, data_types, upload_user_id, upload_date, status_text, claim_user_id, claim_date_time, submission_uuid
    from staging_tbl_clearinghouse_submissions;

drop table staging_tbl_clearinghouse_submissions;
