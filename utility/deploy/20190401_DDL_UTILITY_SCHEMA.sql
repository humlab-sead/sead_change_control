...
begin;
    ...
    create or replace function sead_utility.singularize_en(word text) -- Generated by ChatGPT5
    ...
    create or replace function sead_utility.table_name_to_entity_name(p_tablename text)
    ...
    declare
        ...
        result text;
    begin
        base  := regexp_replace(lower(p_table), '^tbl_', '');
    ...
    */
    ...
    drop function if exists sead_utility.get_sample_graph(p_physical_sample_id int);
    create or replace function sead_utility.get_sample_graph(p_physical_sample_id int)
      returns jsonb as $ok$
    ...
    begin
      with
        ...
        t as (
          ...
          from (
            /* nodes */
            select jsonb_build_object('id', 'site_'||si.site_id, 'entity','Site','attrs',to_jsonb(si)) as node, null::jsonb as edge from si
            union all
            select jsonb_build_object('id','sg_'||sg.sample_group_id,'entity','SampleGroup','attrs',to_jsonb(sg)), null::jsonb as edge from sg
            union all
            select jsonb_build_object('id','ps_'||ps.physical_sample_id,'entity','Sample','attrs',to_jsonb(ps)), null from ps
            union all
            select jsonb_build_object('id','ae_'||ae.analysis_entity_id,'entity','Analysis','attrs',to_jsonb(ae)), null from ae
            union all
            select jsonb_build_object('id','ds_'||ds.dataset_id,'entity','Dataset','attrs',to_jsonb(ds)), null from ds
            union all
            select jsonb_build_object('id','sl_'||sl.location_id,'entity','Location','attrs',to_jsonb(sl)), null from sl
            union all
            select jsonb_build_object('id','f_'||f.feature_id,'entity','Feature','attrs',to_jsonb(f)), null from f
            union all
            select jsonb_build_object('id','p_'||p.project_id,'entity','Project','attrs',to_jsonb(p)), null from tbl_projects p where project_id in (select project_id from p)
            union all
            select jsonb_build_object('id','m_'||m.method_id,'entity','Method','attrs',to_jsonb(m)), null from m
            union all
            select jsonb_build_object('id','b_'||b.biblio_id,'entity','Bibliography','attrs',to_jsonb(b)), null from b
            union all
            select jsonb_build_object('id','av_'||av.analysis_value_id,'entity','AnalysisValue','attrs',to_jsonb(av)), null from av
            union all
            select jsonb_build_object('id','vc_'||vc.value_class_id,'entity','ValueClass','attrs',to_jsonb(vc)), null from vc
            union all -- Abundance / Taxon
            select jsonb_build_object('id','tc_'||tc.abundance_id,'entity','TaxonCount','attrs',to_jsonb(tc)), null from tc
            union all -- Measured Value
            select jsonb_build_object('id','mv_'||mv.measured_value_id,'entity','MeasuredValue','attrs',to_jsonb(mv)), null from mv
            union all -- Taxon
            select jsonb_build_object('id','taxon_'||t.taxon_id,'entity','Taxon','attrs',to_jsonb(t)), null from t
            /* edges */
    ...
    column_sql AS (
        SELECT format(
                -- Include a computed tsvector column in each SELECT
                'select %L AS table_name,
                        %L AS column_name,
                        %I::text AS system_id,
                        %I::text AS value,
                        to_tsvector(''swedish'', unaccent(%I::text)) AS tsv
                 from %I
                 where %I is not null',
                table_name,          -- %L → literal 'tbl_x'
                column_name,         -- %L → literal 'col_y'
                pk_name,             -- %I::text → PK column reference cast to text
                column_name,         -- %I::text → value column reference cast to text
                column_name,         -- %I::text → same column for tsv build
                table_name,          -- %I → FROM table,
                column_name          -- %I → WHERE column IS NOT NULL
            ) AS column_sql
        from sead_utility.table_columns
        join lookup_tables using (table_name)
        ...
        and table_name not in ('tbl_contacts','tbl_biblio','tbl_horizons','tbl_lithology')
    )
    ...
    -- gin index on the precomputed tsv column
    v_sql := 'create index idx_full_text_search
