...
/***************************************************************************
  ...
  Date           
  Description    
  Issue          https://github.com/humlab-sead/sead_change_control/issues/329
  Prerequisites  
  Reviewer
  Approver
  Idempotent     NO
...
create table clearing_house_commit.temp_tbl_biblio as select biblio_id, bugs_reference, date_updated, doi, isbn, notes, title, year, authors, full_reference, url, biblio_uuid from public.tbl_biblio where FALSE;
...
insert into public.tbl_biblio (biblio_id, bugs_reference, date_updated, doi, isbn, notes, title, year, authors, full_reference, url, biblio_uuid)
...
create table clearing_house_commit.temp_tbl_taxonomic_order_systems as select taxonomic_order_system_id, date_updated, system_description, system_name, taxonomic_order_system_uuid from public.tbl_taxonomic_order_systems where FALSE;
...
insert into public.tbl_taxonomic_order_systems (taxonomic_order_system_id, date_updated, system_description, system_name, taxonomic_order_system_uuid)
...
create table clearing_house_commit.temp_tbl_dataset_masters as select master_set_id, contact_id, biblio_id, master_name, master_notes, url, date_updated, master_set_uuid from public.tbl_dataset_masters where FALSE;
...
create table clearing_house_commit.temp_tbl_methods as select method_id, biblio_id, date_updated, description, method_abbrev_or_alt_name, method_group_id, method_name, record_type_id, unit_id, method_uuid from public.tbl_methods where FALSE;
...
create table clearing_house_commit.temp_tbl_value_types as select value_type_id, unit_id, data_type_id, name, base_type, precision, description from public.tbl_value_types where FALSE;
...
create table clearing_house_commit.temp_tbl_value_classes as select value_class_id, value_type_id, method_id, parent_id, name, description from public.tbl_value_classes where FALSE;
...
insert into public.tbl_value_classes (value_class_id, value_type_id, method_id, parent_id, name, description)
...
create table clearing_house_commit.temp_tbl_sites as select site_id, altitude, latitude_dd, longitude_dd, national_site_identifier, site_description, site_name, site_preservation_status_id, date_updated, site_location_accuracy, site_uuid from public.tbl_sites where FALSE;
