/*
 ...
 CHANGES:
 - remove explicit sequence statement, replace with "serial" = "int4 not null default nextval('my_table_my_id_seq'::regclass)"
 - remove drop table statements
 - remove alter sequence owned by
 - remove sequence setval (as this should be startingpoint)
 [ ] Check that all sequences in "sead_staging" has default PostgreSQL naming 'my_table_my_id_seq'
 
 with sead_sequences as (
 SELECT
 nspname AS schema_name,
 relname AS table_name,
 attname AS column_name,
 substring(adsrc FROM '\(''public\.(.*?)''::regclass\)') as constraint_name,
 ...
 from sead_sequences
 where constraint_name <> format('%s_%s_seq', table_name, column_name)
 
 
 [x] Check that all primary keys in "sead_staging" has default PostgreSQL naming '<table>_pkey'
 
 Naming has not been consistent, but a check in sead_change_controls gives that no logic is dependent on pk-constraint name.
 
 SELECT conrelid::regclass AS table_name,
 conname AS constraint_name,
 substring(pg_get_constraintdef(oid) FROM '\((.*?)\)') as column_name
 FROM  pg_constraint
 WHERE contype = 'p'
 AND connamespace = 'public'::regnamespace
 AND conname <> format('%s_pkey', conrelid::regclass)
 ORDER  BY conrelid::regclass::text, contype DESC;
 
 10 names don't follow default naming scheme:
 
 "tbl_analysis_entity_prep_meth_analysis_entity_prep_method_i_seq"
 "tbl_coordinate_method_dimensi_coordinate_method_dimension_i_seq"
 "tbl_project_stage_project_stage_id_seq"
 "tbl_sample_coordinates_sample_coordinates_id_seq"
 "tbl_sample_description_sample_sample_description_sample_gro_seq"
 "tbl_sample_group_description__sample_group_desciption_sampl_seq"
 "tbl_sample_group_description__sample_group_description_type_seq"
 "tbl_sample_location_sampling__sample_location_type_sampling_seq"
 "tbl_sample_location_sampling_contex_sample_location_type_id_seq"
 "tbl_association_types_association_type_id_seq"
 
 None of these names are referenced in sead_change_control
 
 [ ] Check that all foreign keys in "sead_staging"  has default PostgreSQL naming 'my_table_my_id_seq'
